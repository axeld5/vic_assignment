...
from preprocess.data_info import get_bin_mask_from_bbox_list
from sliding_window.detect import detect 

def evaluate_jaccard(img_list, train_bin_masks, rescale_params, hog_desc, use_hog, use_spatial, use_color,
                        clf, threshold, max_size=30*30, plot=False):
    jaccard_scores = [0]*len(img_list)
    start = time.time()
    for i, img in enumerate(img_list):
        _, bounding_boxes = detect(img, rescale_params=rescale_params, hog_desc=hog_desc, use_hog=use_hog, use_spatial=use_spatial, use_color=use_color,
                        clf=clf, threshold=threshold, max_size=max_size, plot=plot)
